...
exports.stravaController = {
    // connect to strava
    stravaConnect: async (req, res) => {
        ...
        try {
            ...
            ;
            const stravaData = await user_model_1.userModel.createStravaConnect(Number(state), is_scope, data);
    ...
    stravaGetData: async (req, res) => {
        ...
        const getNewStravaToken = async (refresh_token) => {
            let url = stravaOAuthUrl;
            url += '?client_id=' + process.env.STRAVA_CLIENT_ID;
            url += '&client_secret=' + process.env.STRAVA_CLIENT_SECRET;
        ...
        const updateStravaToken = async (userid, data) => {
            const { access_token, expires_at, refresh_token } = data;
            try {
                const stravaData = await user_model_1.userModel.updateStravaData(userid, access_token, expires_at, refresh_token);
                if (typeof stravaData.id === 'number') {
        ...
        const getStravaActivities = async (userid, after, befor, access_token) => {
        ...
        try {
            ...
            const { access_token, expires_at, refresh_token, scope } = await user_model_1.userModel.getStravaData(userid);
            if (scope) {
